...
class base {
    ...
    var $post = array();
    var $regular;
    var $statusarray = array('all' => '全部', '0' => '待审核', '1' => '已发布', '2' => '已解决', '4' => '悬赏','6' => '推荐', '9' => '已关闭');
    ...
    function load($model, $base = NULL) {
        $base = $base ? $base : $this;
        if (empty($_ENV[$model])) {
            require MCMS_ROOT . "/model/$model.class.php";
    ...
    function send($uid,$aid=0, $type) { //删除了qid参数，没有question对象了。$aid改为了文章id
    ...
    function checkcode() {
        $this->load('user');
        if (isset($this->post['codenum']) && (strtolower(trim($this->post['codenum'])) != $_ENV['user']->get_code())) {
